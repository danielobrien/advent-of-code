...
fn calculate<T, F>(init: T, calc: F, input: &String) -> [[T; 1000]; 1000]
    where T: Copy,
          F: Fn(T, Op) -> T
{
...
fn solve_p1(input: &String) -> u32 {
    ...
    count(calculate(false, p1_instructions, input), |x| x, |x| {
        if x {
            1
        } else {
            0
...
fn solve_p2(input: &String) -> u32 {
    ...
    count(calculate(0u8, p2_instructions, input),
          |_| true,
          |x| x as u32)
}

fn count<T, F, P>(grid: [[T; 1000]; 1000], predicate: P, weight: F) -> u32
    where T: Copy,
          P: Fn(T) -> bool,
          F: Fn(T) -> u32
{

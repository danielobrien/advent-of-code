...
pub fn solve(input: &String) -> Vec<Result<usize, String>> {
    ...
    for line in input.lines() {
        ...
        ingredients.push(Ingredient {name: arr[0], capacity: c, durability: d, flavour: f,
            texture: t, calories: cal});
    }
    let mut best_score = 0;
    for c in combinations(ingredients, 100) {
        let mut combined_ingredient = Ingredient {name: "combined", capacity: 0, durability: 0, flavour: 0,
            texture: 0, calories: 0};
        for (ing, n) in c {
        ...
        let score = if combined_ingredient.capacity <= 0
            || combined_ingredient.durability <= 0
            || combined_ingredient.flavour <= 0
            || combined_ingredient.texture <= 0 {
            0   
        } else {
            combined_ingredient.capacity * combined_ingredient.durability * combined_ingredient.flavour * combined_ingredient.texture
        } as usize;
        if score > best_score {
            best_score = score;
        }

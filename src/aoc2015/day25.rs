...
pub fn solve(input: &String) -> Vec<Result<u64, String>> {
    let mut v = vec![];
    for line in input.lines() {
        let split = line.trim().split(" ").collect::<Vec<_>>();
        let mut r = split[16].to_string();
        r.pop();
        let row = r.parse::<u64>().unwrap();
        let mut c = split[18].to_string();
        c.pop();
        let col = c.parse::<u64>().unwrap();
        v.push((row, col));
    }
    let mut answer = vec![];
    for (row, col) in v {
        ...
        let mut x = FromPrimitive::from_u64(20151125).unwrap();
        let multiplier = FromPrimitive::from_u64(252533).unwrap();
        let divisor = FromPrimitive::from_u64(33554393).unwrap();
